...
Example output:
{
  ...
  try {
    ...
    logger.debug(`Gemini screening raw response: ${raw.slice(0, 200)}`);
    ...
    logger.error('AI screening failed with error:', err);
    logger.error('This is usually a JSON parsing issue with Gemini response');

    return {
      score: 50,
      summary:
        'AI screening completed with limited analysis. Manual review recommended.',
...
export const chatWithCandidatePool = async (
  query: string,
  candidates: Array<{
    name: string;
    score: number;
  ...
  const candidateList = candidates
    .map(
      (c, i) =>
        `${i + 1}. ${c.name} | Score: ${c.score}/100 | Skills: ${c.skills.join(', ')} | Summary: ${c.summary}`
    )
    .join('\n');
...
RECRUITER QUESTION: ${query}`;
  ...
  try {
    ...
    logger.error('Candidate pool chat failed:', err);
...
export const buildResumeFromScratch = async (userInput: {
  name: string;
  email: string;
  phone: string;
  location: string;
...
Return ONLY a JSON object with this structure:
{
  ...
  try {
    ...
    logger.error('Resume build from scratch failed:', err);
    throw new Error('Failed to generate resume. Please try again.');
  }
};

export const buildResumeFromUpload = async (
...
Return ONLY a JSON object with this exact structure:
{
  ...
  try {
    ...
    logger.error('Resume build from upload failed:', err);
...
export const aiJobSearch = async (
  ...
  matches: Array<{ id: string; relevanceScore: number; reason: string }>;
  suggestion: string;
}> => {
  const jobList = jobs
    .map(
      (j, i) =>
        `${i + 1}. ID:${j.id} | ${j.title} at ${j.company || 'Unknown'} | Skills: ${j.skills.join(', ')} | ${j.isRemote ? 'Remote' : j.location || 'On-site'}`
    )
    .join('\n');
...
Include only jobs with relevanceScore >= 40. Sort by score descending. Max 5 matches.`;
  ...
  try {
    ...
    logger.error('AI job search failed:', err);
    return {
      matches: [],
      suggestion:
        'Try searching with specific skills like "Node.js developer" or "React frontend".',
    };
  }
};
