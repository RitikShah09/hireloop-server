...
exports.aiJobSearch =
  exports.getApplicationStats =
  exports.semanticSearch =
  exports.chatWithPool =
...
exports.applyToJob = (0, errors_1.asyncHandler)(async (req, res) => {
  const {
    ...
    application_validator_1.createApplicationSchema,
    req
  );
  const user = req.user;
  ...
  if (existing) throw new errors_1.AppError('Already applied to this job', 409);
  ...
  emailService
    ...
    .catch((e) =>
      logger_1.logger.error('sendApplicationReceivedEmail failed:', e)
    );
  emailService
    ...
    .catch((e) =>
      logger_1.logger.error('sendNewApplicationAlertEmail failed:', e)
    );
  await Promise.all([
...
exports.getApplications = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  if (user.role === 'CANDIDATE') {
    ...
    if (!candidate) throw new errors_1.AppError('Candidate not found', 404);
    ...
    (0, response_1.sendSuccess)(
      ...
      apps,
      200,
      (0, response_2.buildPaginationMeta)(total, page, limit)
    );
    return;
  }
  const { jobId, status } = req.query;
  if (!jobId) throw new errors_1.AppError('jobId is required', 400);
  ...
  if (!job) throw new errors_1.AppError('Job not found', 404);
  if (page === 1 && !status) {
    const cached = await redis_1.redis.get(
      redis_1.RedisKeys.candidateRanking(jobId)
    );
    if (cached) {
      (0, response_1.sendSuccess)(
        res,
        ...
        JSON.parse(cached),
        200,
        (0, response_2.buildPaginationMeta)(0, page, limit)
      );
      return;
    }
  }
  ...
  if (page === 1 && !status) {
    await redis_1.redis.set(
      ...
      JSON.stringify(apps),
      'EX',
      redis_1.REDIS_TTL.RANKING_CACHE
    );
  }
  (0, response_1.sendSuccess)(
    res,
...
exports.updateApplicationStatus = (0, errors_1.asyncHandler)(
  async (req, res) => {
    const {
      body: { status },
      params,
    } = (0, validate_middleware_1.parseRequest)(
      application_validator_1.updateApplicationStatusSchema,
      req
    );
    const user = req.user;
    const company = await prisma_1.default.company.findUnique({
    ...
    if (!application) throw new errors_1.AppError('Application not found', 404);
    ...
    await Promise.all([
      ...
      redis_1.redis.del(redis_1.RedisKeys.applicationStats(user.userId)),
      redis_1.redis.del(
        redis_1.RedisKeys.applicationStats(application.candidate.userId)
      ),
    ]);
...
exports.aiJobSearch = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  const { aiJobSearch: searchFn } = await Promise.resolve().then(() =>
