...
exports.getCertifications = (0, errors_1.asyncHandler)(async (req, res) => {
  const user = req.user;
  const candidateId = await getCandidateId(user.userId);
  const cacheKey = redis_1.RedisKeys.candidateData(
    candidateId,
    'certifications'
  );
  ...
  await redis_1.redis.set(
    ...
    'EX',
    redis_1.REDIS_TTL.CANDIDATE_DATA
  );
...
exports.addCertification = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await redis_1.redis.del(
    redis_1.RedisKeys.candidateData(candidateId, 'certifications')
  );
...
exports.addWorkExperience = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await redis_1.redis.del(
    redis_1.RedisKeys.candidateData(candidateId, 'experience')
  );
...
exports.addEducation = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await redis_1.redis.del(
    redis_1.RedisKeys.candidateData(candidateId, 'education')
  );
...
exports.addMilestone = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await redis_1.redis.del(
    redis_1.RedisKeys.candidateData(candidateId, 'milestones')
  );
...
exports.getSuggestedJobs = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await redis_1.redis.set(
    ...
    JSON.stringify(jobs),
    'EX',
    redis_1.REDIS_TTL.SUGGESTED_JOBS
  );
