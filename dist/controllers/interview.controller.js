...
exports.scheduleInterview = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await Promise.all([
    ...
    redis_1.redis.del(
      redis_1.RedisKeys.interviewsCache(application.candidate.userId)
    ),
  ]);
  ...
  notificationService
    ...
    .catch((e) =>
      logger_1.logger.error(
        'createNotification (interview scheduled) failed:',
        e
      )
    );
...
exports.getInterviews = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  const cacheKey =
    page === 1 ? redis_1.RedisKeys.interviewsCache(user.userId) : null;
  if (cacheKey) {
...
exports.updateInterview = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  if (!interview) throw new errors_1.AppError('Interview not found', 404);
  ...
  await Promise.all([
    redis_1.redis.del(redis_1.RedisKeys.interviewsCache(user.userId)),
    redis_1.redis.del(
      redis_1.RedisKeys.interviewsCache(interview.application.candidate.userId)
    ),
  ]);
  ...
  if (isCancelled || isRescheduled) {
    ...
    const newDate = scheduledAt ? new Date(scheduledAt) : interview.scheduledAt;
    ...
    notificationService
      ...
      .catch((e) =>
        logger_1.logger.error(
          'createNotification (interview update) failed:',
          e
        )
      );
  }
...
exports.respondToInterview = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  await Promise.all([
    redis_1.redis.del(redis_1.RedisKeys.interviewsCache(user.userId)),
    redis_1.redis.del(
      redis_1.RedisKeys.interviewsCache(
        interview.application.job.company.userId
      )
    ),
  ]);
  ...
  notificationService
    ...
    .catch((e) =>
      logger_1.logger.error('createNotification (interview respond) failed:', e)
    );
  (0, response_1.sendSuccess)(
    res,
