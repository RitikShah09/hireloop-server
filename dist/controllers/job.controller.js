...
const invalidateJobCaches = async (companyId, jobId, slug) => {
  await Promise.all([
    ...
    jobId
      ? redis_1.redis.del(redis_1.RedisKeys.jobCache(jobId))
      : Promise.resolve(),
    slug
      ? redis_1.redis.del(redis_1.RedisKeys.jobSlugCache(slug))
      : Promise.resolve(),
  ]);
};
exports.createJob = (0, errors_1.asyncHandler)(async (req, res) => {
  const { body } = (0, validate_middleware_1.parseRequest)(
    job_validator_1.createJobSchema,
    req
  );
  const user = req.user;
...
exports.updateJob = (0, errors_1.asyncHandler)(async (req, res) => {
  const { body, params } = (0, validate_middleware_1.parseRequest)(
    job_validator_1.updateJobSchema,
    req
  );
  const user = req.user;
  const company = await prisma_1.default.company.findUnique({
...
exports.listJobs = (0, errors_1.asyncHandler)(async (req, res) => {
  const {
    ...
    job_validator_1.jobQuerySchema,
    req
  );
  ...
  const cacheKey = redis_1.RedisKeys.jobListCache(
...
exports.getCompanyJobs = (0, errors_1.asyncHandler)(async (req, res) => {
  ...
  if (!company) throw new errors_1.AppError('Company not found', 404);
  const cacheKey = redis_1.RedisKeys.companyJobsCache(
    company.id,
    paramKey({ page, limit })
  );
  const cached = await redis_1.redis.get(cacheKey);
