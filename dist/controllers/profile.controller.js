...
exports.getPublicCompanyProfile = exports.uploadCompanyLogo = exports.updateCompanyProfile = exports.getCompanyProfile = exports.uploadCandidateAvatar = exports.updateCandidateProfile = exports.getCandidateProfile = void 0;
...
exports.getCandidateProfile = (0, errors_1.asyncHandler)(async (req, res) => {
    ...
    await redis_1.redis.set(cacheKey, JSON.stringify(candidate), 'EX', redis_1.REDIS_TTL.PROFILE);
    (0, response_1.sendSuccess)(res, 'Profile fetched', candidate);
});
exports.updateCandidateProfile = (0, errors_1.asyncHandler)(async (req, res) => {
    const user = req.user;
...
exports.uploadCandidateAvatar = (0, errors_1.asyncHandler)(async (req, res) => {
    const user = req.user;
    if (!req.file)
        throw new errors_1.AppError('Image is required', 400);
    const candidate = await prisma_1.default.candidate.findUnique({
        where: { userId: user.userId },
    });
    if (!candidate)
    ...
    const { url, fileId } = await imagekitService.uploadCandidateAvatar(req.file.buffer, req.file.originalname, candidate.id);
    const updated = await prisma_1.default.candidate.update({
        where: { userId: user.userId },
        data: { avatarUrl: url, avatarFileId: fileId },
    });
    await redis_1.redis.del(redis_1.RedisKeys.candidateProfile(user.userId));
    (0, response_1.sendSuccess)(res, 'Avatar updated', { avatarUrl: updated.avatarUrl });
});
exports.getCompanyProfile = (0, errors_1.asyncHandler)(async (req, res) => {
    ...
    const cached = await redis_1.redis.get(cacheKey);
    if (cached) {
        (0, response_1.sendSuccess)(res, 'Company profile fetched', JSON.parse(cached));
        return;
    }
    ...
    await redis_1.redis.set(cacheKey, JSON.stringify(company), 'EX', redis_1.REDIS_TTL.PROFILE);
...
exports.uploadCompanyLogo = (0, errors_1.asyncHandler)(async (req, res) => {
    ...
    const { url, fileId } = await imagekitService.uploadCompanyLogo(req.file.buffer, req.file.originalname, company.id);
    ...
    (0, response_1.sendSuccess)(res, 'Logo updated', { logoUrl: updated.logoUrl });
});
exports.getPublicCompanyProfile = (0, errors_1.asyncHandler)(async (req, res) => {
    ...
    if (!company)
        throw new errors_1.AppError('Company not found', 404);
    await redis_1.redis.set(cacheKey, JSON.stringify(company), 'EX', redis_1.REDIS_TTL.PUBLIC_PROFILE);
