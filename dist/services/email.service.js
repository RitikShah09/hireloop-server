...
exports.sendScreeningCompleteEmail = exports.sendNewApplicationAlertEmail = exports.sendApplicationReceivedEmail = exports.sendApplicationEmail = exports.sendEmail = exports.sendInterviewEmail = exports.otpEmailTemplate = void 0;
...
const heading = (text) => `<h2 style="margin:0 0 16px;color:${B.text};font-size:20px;font-weight:700;line-height:1.3;letter-spacing:-0.3px;font-family:'Inter',Arial,sans-serif;">${text}</h2>`;
const para = (text, muted = false) => `<p style="margin:0 0 14px;color:${muted ? B.textMuted : B.text};font-size:15px;line-height:1.75;font-family:'Inter',Arial,sans-serif;">${text}</p>`;
...
const divider = () => `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:24px 0;"><tr><td style="border-top:1px solid ${B.border};"></td></tr></table>`;
const callout = (text, color = B.primary, bg = B.primaryLight) => `<div style="background:${bg};border-left:3px solid ${color};border-radius:0 6px 6px 0;padding:14px 18px;margin:20px 0;">
...
const otpEmailTemplate = (opts) => emailWrapper(`<div style="text-align:center;">
...
const interviewCalloutText = {
    scheduled: 'Please make sure you are available at the scheduled time and join a few minutes early. Good luck!',
    cancelled: 'This interview has been cancelled. Please reach out to the company if you have any questions.',
    rescheduled: 'Your interview has been moved. Please check the updated date and time above.',
};
const sendInterviewEmail = async (opts) => {
    ...
    ${infoTable(infoRow('Date &amp; Time', `<strong>${dateStr} IST</strong>`) +
        infoRow('Duration', `${opts.durationMins} minutes`) +
        infoRow('Mode', modeLabel) +
        (opts.meetLink
            ? infoRow('Meet Link', `<a href="${opts.meetLink}" style="color:${B.primary};text-decoration:underline;">${opts.meetLink}</a>`)
            : '') +
