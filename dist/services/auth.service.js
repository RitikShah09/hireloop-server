...
const registerUser = async (input, meta) => {
  ...
  if (existing) throw new errors_1.AppError('Email already in use', 409);
...
const loginUser = async (input, meta) => {
  ...
  if (!user) throw new errors_1.AppError('Invalid credentials', 401);
  if (user.isBlocked)
  ...
  if (!isValid) throw new errors_1.AppError('Invalid credentials', 401);
  const sessionId = (0, uuid_1.v4)();
  const { accessToken, refreshToken } = (0, token_1.generateTokens)({
    userId: user.id,
    role: user.role,
...
const refreshAccessToken = async (refreshToken) => {
  const blacklisted = await (0, token_1.isRefreshTokenBlacklisted)(
    refreshToken
  );
  if (blacklisted) throw new errors_1.AppError('Refresh token revoked', 401);
  let payload;
  try {
  ...
  if (!active) throw new errors_1.AppError('Session expired', 401);
  const stored = await prisma_1.default.refreshToken.findUnique({
    where: { token: refreshToken },
  });
  if (!stored) throw new errors_1.AppError('Refresh token not found', 401);
...
const revokeSession = async (userId, sessionId) => {
  ...
  if (!session) throw new errors_1.AppError('Session not found', 404);
  await (0, token_1.invalidateSession)(sessionId, userId);
  await prisma_1.default.$transaction([
...
const getUserProfile = async (userId) => {
  ...
  if (!user) throw new errors_1.AppError('User not found', 404);
  return user;
};
