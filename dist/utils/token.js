...
exports.isSessionActive =
  exports.invalidateAllSessions =
  exports.invalidateSession =
  exports.storeSession =
  exports.isRefreshTokenBlacklisted =
  exports.isAccessTokenBlacklisted =
  exports.blacklistRefreshToken =
  exports.blacklistAccessToken =
...
const blacklistAccessToken = async (jti) => {
  await redis_1.redis.set(
    redis_1.RedisKeys.accessTokenBlacklist(jti),
    '1',
    'EX',
    redis_1.REDIS_TTL.ACCESS_TOKEN
  );
};
exports.blacklistAccessToken = blacklistAccessToken;
const blacklistRefreshToken = async (token) => {
  await redis_1.redis.set(
    redis_1.RedisKeys.refreshTokenBlacklist(token),
    '1',
    'EX',
    redis_1.REDIS_TTL.REFRESH_TOKEN
  );
};
exports.blacklistRefreshToken = blacklistRefreshToken;
const isAccessTokenBlacklisted = async (jti) => {
  const result = await redis_1.redis.get(
    redis_1.RedisKeys.accessTokenBlacklist(jti)
  );
  return result === '1';
};
...
const storeSession = async (sessionId, userId, meta) => {
  await redis_1.redis.set(
    redis_1.RedisKeys.session(sessionId),
    JSON.stringify({ userId, ...meta, createdAt: Date.now() }),
    'EX',
    redis_1.REDIS_TTL.SESSION
  );
  await redis_1.redis.sadd(redis_1.RedisKeys.userSessions(userId), sessionId);
  await redis_1.redis.expire(
    redis_1.RedisKeys.userSessions(userId),
    redis_1.REDIS_TTL.SESSION
  );
};
...
const invalidateAllSessions = async (userId) => {
  const sessions = await redis_1.redis.smembers(
    redis_1.RedisKeys.userSessions(userId)
  );
  if (sessions.length) {
